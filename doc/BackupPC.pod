...
This documentation describes BackupPC version 4.0.0alpha2,
released on 15 Sep 2013.
...
backup "n" contains the changes relative to prior backup "n+1".
...
and the opposite changes are added to backup #4, to keep the "view"
...
The treatment of a "Partial" backup has changed.  To understand this,
...
If some files were backed up, the last backup is marked "partial".
...
contains an "attrib" file.  The attrib file comprises a header and
the MD5 digest of the real attribute file contents in the pool.

The real attrib file in the pool contains the directory contents: for each,
file, that means the meta data, xattrs and the MD5 digest of the file
contents.

=item *

A modified rsync 3.0.9, called rsync_bpc, is used on the server side, with a
C code layer that emulates all the file-system OS calls to be compatible with
...
lists of MD5 digests, and corresponding counts deltas.
...
BackupPC_link is removed since it is not needed any more.
...
A new utility BackupPC_ls acts like "ls -l", showing accurate
...
etc refer to *Filled* backups, and "IncrKeepPeriod/IncrKeepCnt" refer
to "Unfilled" backups.
...
Some clever future work: the concepts of full/incr rsync can be completely
   ...
   rsync whole-file MD5 digest matches the V4 digest, so a full backup
   could actually just check the whole-file MD5 digest.  Alternatively,
   a "hybrid" could be to make --ignore-times random per file, based
...
The perl module BackupPC::XS is required, and several others, 
Archive::Zip, XML::RSS, File::Listing, Net::FTP, Net::FTP::RetrHandle,
...
in this module.  You can run "perldoc BackupPC::XS" to see if this module
is installed.
BackupPC::XS is available from L<http://backuppc.sourceforge.net> and CPAN.
...
BackupPC-4.0.0alpha2.tar.gz, run these commands as root:

    tar zxf BackupPC-4.0.0alpha2.tar.gz
...
available, eg: BackupPC-4.0.0alpha2pl2.diff, you should apply
the patch after extracting the tar file:

     # fetch BackupPC-4.0.0alpha2.tar.gz
     # fetch BackupPC-4.0.0alpha2pl2.diff
     tar zxf BackupPC-4.0.0alpha2.tar.gz
     cd BackupPC-4.0.0alpha2
     patch -p0 < ../BackupPC-4.0.0alpha2pl2.diff
     perl configure.pl
...
FTP Xfer Method is not yet supported in V4.
...
hardlinks include the GNU cp program with the -a option and rsync
...
=item $Conf{PoolSizeNightlyUpdatePeriod} = 16;
