...
Jest \$hostCntGood hostów, które mają kopie zapasowe na sumę \${fullSizeTot}GB
<p>
...
# -----------------------------------
$Lang{Pool_Stat} = <<EOF;
  <li>Pula to \${poolSize}GB zawiera \$info->{"\${name}FileCnt"} plików oraz \$info->{"\${name}DirCnt"} katalogów (stan z \$poolTime),
  <li>Haszowanie puli daje \$info->{"\${name}FileCntRep"} powtarzających się plików z najdłuższym łańcuchem \$info->{"\${name}FileRepMax"},
  <li>Nocne czyszczenie usunęło \$info->{"\${name}FileCntRm"} plików o rozmiarze \${poolRmSize}GB (około \$poolTime),
EOF
...
<form action="\$MyURL" method="post">
  ...
  \$hiddenStr
  <input type="hidden" value="\$In{action}" name="action">
  <input type="checkbox" value="1" name="relative" checked> Stworzyć archiwum powiązane z\${EscHTML(\$pathHdr eq "" ? "/" : \$pathHdr)} (inaczej będzie zawierać pełne ścieżki do plików).
  <br>
...
<table class="tableStnd" border cellspacing="1" cellpadding="3">
  <tr class="tableheader">
    ...
    <td align="center">Wiek (dni)</td>
    <td align="center">Położenie kopii na dysku</td>
  </tr>
  \$str
</table>
<p>
...
<form name="form0" method="post" action="\$MyURL">
  ...
  <ul>
    <li>Przeglądasz kopię nr #\$num, która zaczęła się około \$backupTime (\$backupAge dni temu), \$filledBackup</li>
    <li>Wpisz adres: <input type="text" name="dir" size="50" maxlength="4096" value="\${EscHTML(\$dir)}"> <input type="submit" value="\$Lang->{Go}" name="Submit"></li>
...
<ul>
  ...
  <li>Pliki z tą samą zawartością pomiędzy różnymi kopiami mają ten sam numer wersji.</li>
...
$Lang{Wrong_user__my_userid_is___} =
              "Zły użytkownik: mój userid to \$>, a nie \$uid"
            . "(\$Conf{BackupPCUser})\n";
...
$Lang{Unable_to_open__file__configuration_problem} = "Nie można otworzyć \$file: problem z konfiguracja ?";
$Lang{Only_privileged_users_can_view_log_or_config_files} = "Tylko uprzywilejowani użytkownicy mogą przeglądać logi/pliki konf.";
...
$Lang{You_haven_t_selected_any_files__please_go_Back_to} = "Nie zaznaczyłeś żadnych plików; proszę cofnąć się do"
                . " zaznaczanych plików.";
$Lang{You_haven_t_selected_any_hosts} = "Nie zaznaczyłeś żadnego hosta; proszę cofnij się"
...
$Lang{Backup_requested_on_DHCP__host} = "Kopia zażądana na hoście DHCP \$host (\$In{hostIP}) przez"
...
<li>Host \$host jest zakolejkowany$Lang{Visit_this_directory_in_backup} = <<EOF;
 w kolejce użytkownika (kopia zostanie wykonana wkrótce).</li>
EOF
...
<li>Ponieważ \$host jest w sieci od co najmniej \$Conf{BlackoutGoodCnt} razy, nie zostanie utworzona kopia zapasowa \$blackoutStr.</li>
EOF
...
$Lang{checkAll} = <<EOF;
<tr>
  <td class="fviewborder"><input type="checkbox" name="allFiles" onClick="return checkAll('allFiles');">&nbsp;Select all</td>
  <td colspan="5" align="center" class="fviewborder"><input type="submit" name="Submit" value="Restore selected files"></td>
</tr>
EOF
...
<tr>
  <td class="fviewborder"><input type="checkbox" name="allFiles" onClick="return checkAll('allFiles');">&nbsp;Select all</td>
  <td colspan="2" align="center" class="fviewborder"><input type="submit" name="Submit" value="Archive selected hosts"></td>
</tr>
EOF
...
$Lang{Status_link_pending} = "link w trakcie";
...
$Lang{Only_privileged_users_can_edit_config_files} = "Tylko uprzywilejowani użytkownicy mogą edytować pliki konfiguracyjne.";
