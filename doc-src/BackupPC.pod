...
passwords are stored. They are in plain text. As described in the 
[Step 3: Setting up config.pl](#step-3-setting-up-configpl), there are 
four ways to tell BackupPC the smb share password (manually setting 
an environment variable, setting the environment variable in 
/etc/init.d/backuppc, putting the password in __TOPDIR__/conf/config.pl, 
or putting the password in __TOPDIR__/pc/$host/config.pl). In the 
latter 3 cases the smb share password appears in plain text in a file.
