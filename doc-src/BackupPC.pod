...
the MD5 digest, which is used to look up the file in the pool.
...
on the server side (in place of File::RsyncP), with C code layer to interface
...
This page has links to the current documentation, github and general information.
...
You can also download the distribution from L<http://backuppc.sourceforge.net>.
Hit the "Code" button, then select the "backuppc" or "backuppc-beta" package and
...
and also CPAN and L<http://backuppc.sourceforge.net>.
...
(Note, we should move this to the Github Wiki.)
