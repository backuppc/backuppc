...
__RELEASEDATE__.  A copy of this document is contained in the BackupPC
distribution.  It B<may> be more recent than what you are reading now.

=head2 Overview

BackupPC is a high-performance, enterprise-grade system for backing up Unix,
Linux, WinXX, and macOS PCs, desktops and laptops to a server's disk. BackupPC
...
to extract backup data. On linux, unix or macOS clients, rsync, tar (over
...
This was the first release of version 4, a significant rewrite of BackupPC.
This section provides a short overview of the changes and features.
...
BackupPC_fsck cannot run when BackupPC is running.

=item *

When BackupPC_refCountUpdate updates the overall reference counts, it
removes pool files that have a reference count of zero and whose mtime
is more than seven days in the past.  To avoid race conditions, it
uses a two-phase process.  It first flags files that have zero
reference counts using one of the file attributes.  The next time it
runs (typically 24 hours later, but of course this depends on the
configuration and perhaps operational things), any flagged files that
still have zero reference count and qualifying mtime are then removed.
The rest of the code knows not to use flagged pool files to avoid race
conditions.  If $Conf{XferLogLevel} >= 3, or if transfer errors occur,
then BackupPC_refCountUpdate will log the number of files skipped for
deletion and the number of blocks occupied by those files in total.
...
The perl module BackupPC::XS (version >= 0.63) , Time::ParseDate, CGI
and SCGI are required.  Several others, Archive::Zip, File::Listing,
JSON::XS, Net::FTP, Net::FTP::RetrHandle, Net::FTP::AutoReconnect and
XML::RSS are recommended.

Try "perldoc BackupPC::XS" and e.g. "perldoc Archive::Zip" to see if
you have these modules.  If not, check if your distribution has them
packaged.  It's probably easiest if you can get them from there, and
the distribution will then look after any updates which come along
later. If your distribution doesn't have them packaged, fetch them
from their homes at L<https://github.com/backuppc/backuppc-xs> and
L<http://www.cpan.org> respectively.  See the instructions within the
downloaded package or below for how to build and install them.

The CGI Perl module is required for the http/cgi user interface. CGI
was a core module, but from version 5.22 Perl no longer ships with it
and you may need to install it as well.  The SCGI module is needed in
cases where you're using the SCGI 'shim' between BackupPC and Apache.
You can disable the use of SCGIU in the BackupPC configuration file.
...
Starting with version 4, BackupPC no longer uses hardlinks for storage of
...
package manager is usually the best approach.

For example the Debian package, supported by Axel Beckert, Tobias
Frost, Jonathan Wiltshire and formerly Ludovic Drolez, can be found at
L<http://packages.debian.org/backuppc> and is included in the current
stable Debian release.  On Debian, the command:

    apt-get install backuppc

not only installs backuppc-rsync and libbackuppc-xs-perl (these are
essential to a working BackupPC installation) but also installs Apache
and numerous supporting packages which, although they are not strictly
necessary to run BackupPC, make it much easier to control and monitor
the backup system until you become familiar with it.

Assuming the apt-get command works OK, you can skip to Step 3.
...
L<http://www.cpan.org>. There is no need to install this module if you don't
...
from L<http://www.cpan.org>. There is no need to install this module if you
...
particular, you will need to decide whether to use smb, tar, rsync or ftp
...
rsync or tar are the preferred methods for linux/unix/macOS clients.
...
later, the native macOS tar works, and also supports resource forks.  Another
option is xtar.  For many years Apple supplied an old version of rsync because
of licensing issues (later versions of rsync moved from GPL 2.x to GPL3.x) and
although it was patched for various issues it was still based on 2006 code.
With the release of Sequoia in June 2024 Apple replaced rsync with openrsync,
an alternative which like the older patched rsync from Apple is not entirely
compatible with what one might call 'standard' rsync.  It's possible for the
rsync used by BackupPC to work with openrsync but you will need to modify some
of the options in the BackupPC configuration.  At least remove '--protect-args'
because this option is not supported by any Apple-supplied version of rsync.
A better option on macOS would probably be to install a recent, genuine rsync.
...
backups to 4.x storage format, eliminating hardlinks.  This allows you to
...
Users can edit their host-specific configuration if enabled via
...
Axel Beckert, Jonathan Wiltshire and Tobias Frost support the Debian
BackupPC package.
